...
public class WoodGongFaBase extends Item {
    private final String fileName;
    public static WoodGongFaBase INSTANCE = new WoodGongFaBase("template.json", "wood_gongfa");

    public WoodGongFaBase(String fileName, String registryName){
    ...
    public ActionResult<ItemStack> onItemRightClick(@Nonnull World worldIn, @Nonnull EntityPlayer playerIn, @Nonnull EnumHand handIn) {
        if (GuiScreen.isShiftKeyDown()) {
            if (!worldIn.isRemote) {
                try {
                    Minecraft.getMinecraft().displayGuiScreen(new GongFaInfo("wood/" + fileName, 30));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
            ...
            PutInInventoryHelper.putInInventory(worldIn, GongFaInventory.instance, heldItem, "木");
        }
